...
// Using abort() for NOTREACHED() doesn't support streaming arguments. For a more complete implementation we could use LOG(FATAL) but that is currently not annotated as [[noreturn]] because ~LogMessage is not. See TODO in base/logging.h.
