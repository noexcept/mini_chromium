...
#if !defined(MAC_OS_X_VERSION_10_12) || \
    MAC_OS_X_VERSION_MIN_REQUIRED < MAC_OS_X_VERSION_10_12
    // Use ASL when this might run on pre-10.12 systems. os_log was introduced
    // in 10.12.
...
#undef ASL_LEVEL_STR_X
...
#else
    // Use os_log when this will only run on 10.12 and later. ASL is deprecated
    // in 10.12.
    ...
    os_log_type_t os_log_type = [](LogSeverity severity) {
      switch (severity) {
        case LOG_INFO:
