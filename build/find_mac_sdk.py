...
def _DidNotMeetMinimumSDKVersion(params):
...
def _FindSDKWithMinimumVersion(minimum_sdk_version_str):
  ...
  if xcodebuild_showsdks_subprocess.returncode == 0:
    ...
    if len(sdk_version_strs) == 0:
      _DidNotMeetMinimumSDKVersion({'minimum': minimum_sdk_version_str})
...
def main(args):
  ...
  if parsed.path is not None:
    sdk_version = _SDKVersion(parsed.path)
  ...
  # This is redundant if _FindSDKWithMinimumVersion() was called, but it’s
  # necessary if --minimum was specified along with --exact or --path.
  if parsed.minimum is not None and sdk_version < _AsVersion(parsed.minimum):
    _DidNotMeetMinimumSDKVersion({'exact': parsed.exact,
