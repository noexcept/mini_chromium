...
class WinTool(object):
  ...
  def ExecGetVisualStudioData(self, outdir, *args):
    install_dir, script_path = None, None
    if len(args) and args[0]:
      # Use an explicitly specified toolchain path, if provided.
      setenv_path = os.path.join('win_sdk', 'bin', 'SetEnv.cmd')
      if os.path.exists(os.path.join(args[0], setenv_path)):
        install_dir, script_path = args[0], setenv_path
